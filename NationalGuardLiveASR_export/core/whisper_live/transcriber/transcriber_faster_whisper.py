...
def merge_segments(segments, vad_options):
    """
    Merge adjacent segments based on min_silence_duration_ms.
    """
    if not segments:
        return []

    # vad_options.min_silence_duration_ms is in milliseconds
    # get_speech_timestamps returns timestamps in samples (at 16kHz)
    min_silence_samples = vad_options.min_silence_duration_ms * 16
    ...
    for segment in segments[1:]:
        if segment["start"] - current_end <= min_silence_samples:
