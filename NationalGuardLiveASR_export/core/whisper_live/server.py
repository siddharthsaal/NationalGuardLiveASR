...
class TranscriptionServer:
    ...
    def get_audio_from_websocket(self, websocket):
        ...
        if isinstance(frame_data, str):
            if frame_data == "END_OF_AUDIO":
                return False
            # If it's some other string, it's unexpected here
            return True # Continue loop, maybe more data coming? Or should we skip?
        
        if frame_data == b"END_OF_AUDIO":
            return False
        
        audio_frame = np.frombuffer(frame_data, dtype=np.float32)
        return audio_frame
    ...
    def process_audio_frames(self, websocket):
        ...
        if frame_np is None: # Should not happen with current logic but for safety
             return True
    ...
    def cleanup(self, websocket):
        ...
        if client:
            ...
            if hasattr(client, 'trans_thread') and client.trans_thread:
                logging.info(f"Waiting for transcription thread of client {client.client_uid}...")
                client.trans_thread.join(timeout=5.0)
